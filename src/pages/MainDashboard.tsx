          ...
          {active.length > 0 ? (
              ...
              {active.map((event) => (
                ...
                >
                  <div className="flex items-center gap-1">
                    <span className="text-[8px] font-black text-green-500 uppercase tracking-widest animate-pulse">
                      Ativo
                    </span>
                    <ExternalLink
                      size={8}
            ...
            >
              ...
              <h2 className="text-xs font-bold text-white uppercase group-hover:text-[#d4af37] transition-colors">
                {next.name} ({next.nextStart.getDate()}/
                {next.nextStart.getMonth() + 1})
              </h2>
            </a>
          )}
        </div>
        ...
        <div className="flex flex-col items-center">
          {moon.isFullMoon ? (
            ...
            <div className="flex flex-col items-center opacity-60">
              <span className="text-[8px] font-bold text-[#c4b08a] uppercase tracking-widest text-center">
                Lua Cheia em
              </span>
              <h2 className="text-xs font-bold text-white uppercase">
                {moon.daysUntilNext} Dias
              </h2>
            </div>
          )}
        </div>
...
function MainDashboard() {
  ...
  const [bazaarMetadata, setBazaarMetadata] = useState<
    BazaarResponse['metadata']
  ...
  const [sortBy, setSortBy] = useState<
    'price_asc' | 'price_desc' | 'level_desc' | 'level_asc'
  >('price_desc');
  ...
  const handleShowBazaar = useCallback(
    async (page: number = 1) => {
      setIsLoading(true);
      const formattedWorld = activeFilters.world.trim()
        ? activeFilters.world.charAt(0).toUpperCase() +
          activeFilters.world.slice(1).toLowerCase()
        : undefined;
        ...
        const result = await getTibiaBazaar(
          {
            ...
            minLevel: activeFilters.minLevel
              ? parseInt(activeFilters.minLevel)
              : undefined,
            maxLevel: activeFilters.maxLevel
              ? parseInt(activeFilters.maxLevel)
              : undefined,
            sort: sortBy,
          },
          page,
            ...
            {viewMode === 'bazaar' && (
              <div className="space-y-4 animate-in fade-in duration-500">
                <div className="bg-black/60 border border-[#d4af37]/30 rounded-xl p-6 backdrop-blur-md">
                  ...
                  <div
                    className={`space-y-4 transition-all ${showFilters ? 'opacity-100 block' : 'hidden'}`}
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[9px] uppercase text-[#d4af37]/70 font-bold">
                          Nickname (Busca Global)
                        </label>
                        <Input
                          placeholder="Ex: Lord Paulistinha"
                          value={bazaarFilters.nickname}
                          onChange={(e) =>
                            setBazaarFilters({
                              ...bazaarFilters,
                          ...
                          className="h-9 bg-black/40 border-[#d4af37]/20 text-xs text-white"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] uppercase text-[#d4af37]/70 font-bold">
                        ...
                        <select
                          ...
                          className="w-full bg-black/80 border border-[#d4af37]/20 rounded p-2 text-xs text-white outline-none h-9"
                        >
                ...
                <div className="flex justify-end">
                  <div className="flex items-center gap-2 bg-black border border-[#d4af37]/40 rounded px-2 h-9 shadow-inner shadow-[#d4af37]/10">
                    ...
                    <select
                      ...
                      className="bg-black text-[10px] text-white outline-none cursor-pointer uppercase font-black tracking-widest"
                    >
                ...
                <div className="grid gap-4">
                  {bazaar.length > 0 ? (
                        ...
                        <Card className="bg-black/80 border-[#d4af37]/30 backdrop-blur-md hover:border-[#d4af37]/80 transition-all duration-300 group overflow-hidden shadow-lg cursor-pointer">
                          <CardContent className="p-0 flex flex-col md:flex-row">
                            <div className="p-4 flex items-center gap-5 flex-1">
                              <div className="relative bg-linear-to-b from-[#d4af37]/20 to-transparent p-1.5 rounded-lg border border-[#d4af37]/20">
                                <div className="bg-black/40 rounded-md p-1">
                                  <img
                                    src={
                                      item.outfit_url ||
                                      '/placeholder_outfit.png'
                                    }
                                    alt=""
                                    className="h-20 w-20 object-contain drop-shadow-[0_0_8px_rgba(212,175,55,0.3)]"
                                  />
                                </div>
                                <div className="absolute -top-2 -left-2 bg-[#d4af37] text-black text-[11px] px-2 py-0.5 font-bold rounded border border-black/20 shadow-md">
                                  LV {item.level}
                                </div>
                              </div>
                              <div className="flex-1">
                                ...
                                <p className="text-[12px] text-[#c4b08a]/80 font-semibold tracking-wide uppercase">
                                  {item.vocation}{' '}
                                ...
                                <div className="flex gap-2 mt-4 flex-wrap">
                                  {item.skills.map(
                                    (skill: string, sIdx: number) => {
                                      ...
                                      return (
                                        <div
                                          key={sIdx}
                                          className="bg-black/40 px-2.5 py-1.5 rounded border border-[#d4af37]/20 flex flex-col items-center min-w-10.5"
                                        >
                                          <span className="text-[8px] uppercase text-[#c4b08a] opacity-70 font-black">
                                            {name}
                                          </span>
                                  ...
                                  {item.extras?.length > 0 && (
                                    <div
                                      className="bg-blue-500/10 px-2.5 py-1.5 rounded border border-blue-500/30 flex items-center gap-1.5 cursor-help"
                                      title={item.extras.join('\n')}
                                    >
                                      <Star
                                        size={12}
                            ...
                            <div className="bg-[#d4af37]/5 p-5 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l border-[#d4af37]/10 min-w-45">
                              <div className="text-right">
                                <p className="text-[10px] uppercase tracking-widest text-[#d4af37]/70 font-black mb-1">
                                  Lance Atual
                                </p>
                                <div className="flex items-center gap-2 text-[#d4af37] font-black text-2xl">
                                  <Coins size={20} />{' '}
                    ...
                    <div className="text-center py-20 bg-black/20 rounded-xl border border-dashed border-[#d4af37]/20">
                      <p className="opacity-50 italic text-base text-[#c4b08a]">
                  ...
                  <div className="flex flex-col items-center gap-4 pt-8">
                    <div className="flex items-center justify-center gap-3">
                      <GlassButton
                        variant="secondary"
                        onClick={() =>
                          handleShowBazaar(bazaarMetadata.page - 1)
                        }
                        disabled={bazaarMetadata.page === 1 || isLoading}
                        className="px-3 h-10"
                      >
                        <ChevronLeft size={18} />
                      </GlassButton>
                      ...
                      >
                        <span className="text-[10px] font-medieval uppercase text-[#d4af37]/60">
                          Pág.
                        </span>
                        <input
                          type="number"
                        ...
                        />
                        <span className="text-[10px] font-medieval uppercase text-[#d4af37]/60">
                          de {bazaarMetadata.totalPages}
                        </span>
                        <button
                          type="submit"
                          disabled={isLoading}
                          className="ml-1 hover:text-[#d4af37] transition-colors group"
                        >
                          <Zap
                            size={14}
                            className={`${parseInt(pageInput) > bazaarMetadata.totalPages ? 'text-red-400' : 'text-[#d4af37]/50'} group-hover:text-[#d4af37] transition-colors`}
                          />
                        </button>
                      </form>
                      <GlassButton
                        ...
                        disabled={!bazaarMetadata.hasMore || isLoading}
                        className="px-3 h-10"
                      >
                  ...
                  <Card className="bg-black/40 border-[#d4af37]/20 backdrop-blur-sm">
                    <CardHeader className="border-b border-[#d4af37]/10 py-2 px-4">
                      <CardTitle className="flex items-center gap-2 text-[#d4af37] font-medieval tracking-widest uppercase text-[10px]">
                        <ScrollText className="h-3 w-3" /> Pergaminhos de
                        Consulta
                      </CardTitle>
                    </CardHeader>
